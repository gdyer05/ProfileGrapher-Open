...
$header.Text = "Audits Initiated By Account"
...
Register-Component -relationName "auditsInitBy" -variableName "treeView" -valueOfVar $treeView

Register-EventCallback -EventName "GetUser" -Callback {
    param($user)

    #$baseForm.Controls.Remove($StoredComponents["accountDetails"]["spinner"])
    $treeView = [System.Windows.Forms.TreeView](Get-Component -relationName "auditsInitBy" -variableName "treeView")
    ...
    $job = Start-ThreadJob -ScriptBlock {
        param($searchName)
        Get-MgBetaAuditLogDirectoryAudit -All -Filter "InitiatedBy/User/UserPrincipalName eq '$searchName'"
    } -ArgumentList $user.userPrincipalName

    Register-JobCompletion -Job $job -JobCb {
        param($user, $auditLog)

        $baseForm = (Get-Component -relationName "auditsInitBy" -variableName "baseForm")
        $baseForm.Controls.Remove((Get-Component -relationName "auditsInitBy" -variableName "spinner"))
...
Register-EventCallback -EventName "AwaitData" -Callback {
    ...
    (Get-Component -relationName "auditsInitBy" -variableName "treeView").BackColor = [System.Drawing.Color]::FromArgb(240, 240, 240)
    $baseForm.Controls.Add((Get-Component -relationName "auditsInitBy" -variableName "spinner"))
    (Get-Component -relationName "auditsInitBy" -variableName "spinner").BringToFront()
}

$spinner = Build-LoadingSpinner
Move-LoaderToCenter -loadControl $spinner -controlToCenter $treeView
Register-Component -relationName "auditsInitBy" -variableName "spinner" -valueOfVar $spinner

Register-Component -relationName "auditsInitBy" -variableName "baseForm" -valueOfVar $baseForm